...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    Examples::
        ...
        if extra_loss:
            if input_source_index_mask is None:
                source_index_mask = torch.zeros(
                    (input_ids.shape[0], self.model.shared.num_embeddings), dtype=torch.bool, device=input_ids.device,
                )
        ...
        decoder_output_copy = outputs[0].detach().clone()
        ...
        if lm_labels is not None:
            ...
            if extra_loss:
                labels_copy = torch.LongTensor(
                    [
                        [element if source_index_mask[i][element] else -100 for element in example]
                        for i, example in enumerate(lm_labels.detach().clone())
                    ]
                ).to(lm_logits.device)
        ...
        if self.model.output_attentions:
            all_output_cross_attn = []
            max_attention_per_head = outputs[3][-1].cpu().max(dim=3)
            # encoders are in encoder_outputs[0]
            # decoders are in outputs[0]
            for beam in range(max_attention_per_head[0].shape[0]):
                top_5 = sorted(
                    zip(max_attention_per_head[0][beam].squeeze(), max_attention_per_head[1][beam],), reverse=True,
                )[:5]
                beam_output = [
                    (
                        score,
                        tokenizer.decode(
                            original_input_ids[0][
                                max(index.item() - 5, 0) : min(index.item() + 5, len(original_input_ids[0]))
                            ]
                        ),
                    )
                    for score, index in top_5
                ]
                all_output_cross_attn.append(beam_output)
            return outputs, all_output_cross_attn
